...
public class AdminTeacherController {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public TeacherDeletionImpactResponse getDeletionImpact(@AuthenticationPrincipal TeacherPrincipal principal,
                                                            @PathVariable Long id) {
        return TeacherDeletionImpactResponse.from(
                adminTeacherService.getDeletionImpact(principal.institutionId(), id));
    }

    @DeleteMapping("/api/admin/teachers/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @PreAuthorize("hasRole('ADMIN')")
    public void deleteTeacher(@AuthenticationPrincipal TeacherPrincipal principal, @PathVariable Long id,
